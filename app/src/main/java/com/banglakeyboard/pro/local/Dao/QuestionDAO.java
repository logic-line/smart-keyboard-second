package com.banglakeyboard.pro.local.Dao;
...
import com.banglakeyboard.pro.Models.NotificationData;
...
public interface QuestionDAO {
    ...
    List<Gk> getNotShownQuestions();

    @Query("SELECT * FROM question_table ORDER BY id ASC LIMIT 100")
    List<Gk> getAllQuestions();
    ...
    Config getConfig();
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    void insertOrUpdateUpdateInfo(com.banglakeyboard.pro.Models.Update update);

    @Query("SELECT * FROM update_info_table ORDER BY id DESC LIMIT 1")
    com.banglakeyboard.pro.Models.Update getUpdateInfo();
